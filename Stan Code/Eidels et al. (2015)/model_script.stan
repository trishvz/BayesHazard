...
//
// Note the difference between this code and the code used for the simulated data.  In
// this code, the bin boundaries s are common across conditions, equivalent to the
// common_s variable used for the simulated data.  The dimensionality of the s variable
// is reduced, as is the dimensionality of lags.  The difference between s for these data
// and s for the simulation study is determined in the R code file XXX.
//
...
data {
	...
	int<lower=1> N;        // Sample size
        ...
        vector<lower=0>[J+1] s[Nsub];        // Bin boundaries (quantiles)
}
transformed data {
        // Computation of lags used by functions
        vector<lower=0>[J] lags[Nsub];        // Bin widths

        // For each subject, compute the bin widths as s[j] - s[j-1]
...
parameters {
	...
	// from N(0,1) priors (_raw) and then transformed to N(mu,sigma)
        ...
        vector<lower=0>[levels] var_h[Nsub]; // Variance of epsilon_icj (not in the hierarchy)

        vector[J] ln_h_raw[Nsub,levels];     // Untransformed log hazard rates
...
transformed parameters {
        ...
        vector<lower=-1,upper=1>[levels] phi[Nsub]; // Autoregressive coefficient
        vector<lower=0>[levels] sigma_h[Nsub];      // Standard deviations of epsilon_icj
        ...
        vector[levels] mu[Nsub];                    // Values of mu_ic
        vector[levels] alpha[Nsub];                 // Values of alpha_ic
        vector[J] ln_h[Nsub,levels];                // Values of log hazard rates
...
model {
	...
	for (n in 1:N) {
		...
		t[n] ~ pieceexp(h[K,M],s[K],lags[K]); // compute the likelihood of t[n].
		}
}
